...
export default class Rubik {
  ...
  getGlobalDirection() {
    ...
    const globalDirection = Object.keys(this.direction).reduce((res, item) => {
      const direction = this.direction[item as DirectionItem].applyMatrix4(matrix).sub(globalOrigin);
  ...
  rotateAroundWorldAxis(pos: Vector3, vector: Vector3, rad: number) {
    ...
    matrix4.set(
      ...
      (a * (v * v + w * w) - u * (b * v + c * w)) * (1 - Math.cos(rad)) + (b * w - c * v) * Math.sin(rad),
      u * v * (1 - Math.cos(rad)) + w * Math.sin(rad),
      v * v + (u * u + w * w) * Math.cos(rad),
      v * w * (1 - Math.cos(rad)) - u * Math.sin(rad),
      (b * (u * u + w * w) - v * (a * u + c * w)) * (1 - Math.cos(rad)) + (c * u - a * w) * Math.sin(rad),
      u * w * (1 - Math.cos(rad)) - v * Math.sin(rad),
      v * w * (1 - Math.cos(rad)) + u * Math.sin(rad),
      w * w + (u * u + v * v) * Math.cos(rad),
      (c * (u * u + v * v) - w * (a * u + b * v)) * (1 - Math.cos(rad)) + (a * v - b * u) * Math.sin(rad),
      0,
      0,
      0,
      1
