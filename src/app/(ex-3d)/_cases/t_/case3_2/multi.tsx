...
export default function Case3_2() {
  ...
  const [meshes] = useAsyncState(async () => {
    ...
    function addBoxes(info: Record<string, any>, hueRange: [number, number]) {
      const { file } = info;
    ...
    return fileInfos?.map((info) => addBoxes(info, info.hueRange));
  });
  ...
  useEffect(() => {
    ...
    function requestRenderIfNotRequested() {
      if (!renderRequested) {
        renderRequested = requestRenderIfNotRequested;
