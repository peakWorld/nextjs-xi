...
export default function Case5_1() {
  ...
  useEffect(() => {
    ...
    // 聚光灯
    const lightDirectionLocation = gl.getUniformLocation(program, "u_lightDirection"); // 聚光方向
    ...
    function drawScene() {
      ...
      gl.uniform3fv(lightWorldPositionLocation, lightPos); // 设置光源位置
      gl.uniform3fv(lightColorLocation, [1, 0.6, 0.6]); // 设置光照颜色
      gl.uniform3fv(specularColorLocation, [1, 0.2, 0.2]); // 设置高光颜色
      gl.uniform1f(shininessLocation, shininess[0]); // 设置高光散射/半径

      // 衰减系数<光照衰减常量值 600>
      gl.uniform1f(constantLocation, 1.0);
      gl.uniform1f(linearLocation, 0.007);
      gl.uniform1f(quadraticLocation, 0.0002);

      // 聚光
      gl.uniform1f(innerLimitLocation, Math.cos(glMatrix.toRadian(innerLimit[0])));
      gl.uniform1f(outerLimitLocation, Math.cos(glMatrix.toRadian(outerLimit[0])));
    ...
    gui.add(innerLimit, 0, 0, 90).step(5).name("innerLimit").onChange(drawScene);
    gui.add(outerLimit, 0, 0, 90).step(5).name("outerLimit").onChange(drawScene);
