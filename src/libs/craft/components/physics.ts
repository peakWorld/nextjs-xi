...
class Physics extends Component {
  ...
  // 添加物体
  add({ mesh, body, copyPosition = true, copyQuaternion = true }: MeshPhysicsObjectParams) {
    const obj = new MeshPhysicsObject(mesh, body, copyPosition, copyQuaternion);
    this.craft.physics.world.addBody(body);
    this.meshPhysicsObjects.push(obj);
  }
  // 帧
  tick() {
    const world = this.world;
    const deltaTime = this.craft.clock.deltaTime;
