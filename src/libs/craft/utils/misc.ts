...
// 从hdr贴图中提取envmap
const getEnvmapFromHDRTexture = (renderer: THREE.WebGLRenderer, texture: THREE.Texture) => {
...
// 从场景中提取envmap
const getEnvmapFromScene = (renderer: THREE.WebGLRenderer, scene: THREE.Scene) => {
  const pmremGenerator = new THREE.PMREMGenerator(renderer);
...
// 从mesh上取样微粒位置信息
const sampleParticlesPositionFromMesh = (geometry: THREE.BufferGeometry, count = 10000) => {
...
// 获取position中每个三角形的中心
const getPositionCentroids = (geometry: THREE.BufferGeometry, attrName = "position", centroidName = "aCenter") => {
...
// 创建多边形形状
const createPolygonShape = (points: THREE.Vector2[], config: Partial<CreatePolygonShapeConfig> = {}) => {
...
// 计算透视相机全屏大小
const calcPerspectiveScreenSize = (targetZ = 0, camera: THREE.PerspectiveCamera, aspect: number) => {
  let screenWidth = 1;
  let screenHeight = 1;
...
const exportGLTF = (
  ...
  { filename = "scene", trs = false, onlyVisible = true, binary = false, maxTextureSize = 4096 } = {}
) => {
